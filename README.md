...
```bash
npx @angular/cli@^21 new bark-back --routing --style=css --no-ssr --no-zoneless --test-runner=karma
cd bark-back
```
...
npx ng config 'projects.bark-back.architect.test.options.coverageExclude' '["**/fixtures/**"]'
```

Add Prettier and ES Lint support:

```bash
npx ng add angular-eslint@^21
npx ng add eslint-config-skyux@^14
```

Add `@skyux/packages` to the project, which adds SKY UX stylesheets and sets up upgrades:

```bash
npx ng add @skyux/packages@^14
```
...
npx ng g service services/data/fixtures/mock-data --skip-tests
```
